...
|`entity.py`|класс доменной сущности. Чистый `python`-класс, анемичная модель|
...
|`schema.py`|`pydantic`-схемы объектов `dto` и `response`, класс маппинга данных при передаче между архитектурными слоями|
|`services.py`|бизнес-логика|

### Соглашение об именовании

- названия доменных моделей, `orm`-моделей, таблиц БД, `pydantic`-схем даются в единственном числе
- название `python`-класса доменной сущности дается с суффиксом `Entity` (например `AuthorEntity`, `GenreEntity`)
- названия `orm`-моделей (файлы `models.py`) даются без суффиксов (например `Author`, `User`)
- названия `pydantic`-схем, в т.ч. базовых, от которых потом наследуются схемы конкретных сущностей (файлы `schema.py`) даются с суффиксом `Schema` (например `BaseSchema`, `UserCreateSchema`, `AuthorResponseSchema`)
- название класса с мапперами данных дается с суффиксом `Mappers` (например `AuthorMappers`)
